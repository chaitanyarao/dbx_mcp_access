...
def infer_sql_arguments(tool: ToolDetails, sql: str, settings: Settings) -> dict[str, Any]:
    ...
    for key in properties:
        ...
        elif lowered in {"catalog"}:
            arguments[key] = settings.catalog
        elif lowered in {"schema"}:
...
class QueryService:
    ...
    def test_connection(self) -> dict[str, Any]:
        ...
        return {
            ...
            "selected_sql_tool": sql_tool.name,
    ...
    def preview_table(self, limit: int | None = None) -> dict[str, Any]:
        requested_limit = min(limit or self.settings.query_row_limit, self.settings.query_row_limit)
        sql = f"SELECT * FROM {self.settings.full_table_name} LIMIT {requested_limit}"
        return self.run_query(sql)

    def count_rows(self) -> dict[str, Any]:
        sql = f"SELECT COUNT(*) AS row_count FROM {self.settings.full_table_name}"
        return self.run_query(sql)
