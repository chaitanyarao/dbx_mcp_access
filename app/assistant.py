...
class TableAssistant:
    ...
    def describe_table(self, table_name: str | None) -> list[dict[str, Any]]:
        full_name = self._qualify_table(table_name)
        result = self.query_service.run_query(f"DESCRIBE TABLE {full_name}")
        return result.get("rows") or []

    def answer_question(self, table_name: str | None, question: str) -> AssistantAnswer:
    ...
    def _qualify_table(self, table_name: str | None) -> str:
        if not table_name or not str(table_name).strip():
            raise ValueError(
                "No table is currently selected. This usually means the app could not list any tables in the configured catalog and schema."
            )
        available = self.list_tables()
        simple_name = str(table_name).split(".")[-1]
...
""".strip()
    ...
    def _schema_prompt_text(self, schema_rows: list[dict[str, Any]]) -> str:
        ...
        lines = [f"- {row.get('col_name')}: {row.get('data_type')}" for row in selected]
